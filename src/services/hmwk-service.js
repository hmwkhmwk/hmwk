...
class HmwkService {
    ...
    static async populateHmwkCompletionTrackingBoard(token, hmwkCompletionTrackingBoardId, studentInfo, hmwkName, dueDate){
        try{
            const mondayClient = initMondayClient();
            mondayClient.setToken(token);
            ...
            for(let i=0; i<studentInfo.length; i++){
                ...
                const variables = { hmwkCompletionTrackingBoardId,  hmwkName, studentInfo, column_values};
                await mondayClient.api(query, { variables });
            }
